...
class AlgorithmBase(ABC):
    ...
    def _calculate_penalty_factor(self):
        ...
        self.penalty_factor = \
            PENALTY_FACTOR_BASE * (np.sum(self.dataset[:, 1]) / np.sum(self.dataset[:, 0])) * self.backpack_capacity
    ...
    def evaluate_fitness(self, population):
        ...
        penalties = np.maximum(total_weights - self.backpack_capacity, 0) * self.penalty_factor
