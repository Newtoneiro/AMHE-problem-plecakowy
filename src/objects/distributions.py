...
class NormalDistribution(DistributionKind):
    ...
    def generate_data(self, no_samples: int) -> tuple[np.ndarray]:
        weights = np.random.normal(
            loc=self.mean_weight, scale=np.sqrt(self.variance_weight), size=no_samples
        )
        values = np.random.normal(
            loc=self.mean_value, scale=np.sqrt(self.variance_value), size=no_samples
        )
...
class UniformDistribution(DistributionKind):
    ...
    def generate_data(self, no_samples: int) -> tuple[np.ndarray]:
        weights = np.random.uniform(
            low=self.min_weight, high=self.max_weight, size=no_samples
        )
        values = np.random.uniform(
            low=self.min_value, high=self.max_value, size=no_samples
        )
